...
public class ValidarMetodoPagamento implements ValidadorFazerPedido {
	...
	public void validar(Pedido pedido, Customer customer) {
		...
		if ((metodoPagamento.precisaDeCartao() && cartaoId == null) || (metodoPagamento.podeParcelar() && numeroParcelas == null)) {
			throw new ValidationException("O método de pagamento escolhido foi " + metodoPagamento + ", portanto, é necessário enviar o cartaoId do customer e o numeroParcelas juntamente.");
		}
		if ((!metodoPagamento.precisaDeCartao() && cartaoId != null) || (!metodoPagamento.podeParcelar() && numeroParcelas != null)) {
			throw new ValidationException("O método de pagamento escolhido foi " + metodoPagamento + ", portanto, você não deve enviar um cartaoId nem o numeroParcelas junto.");
		}
		...
		if (cartaoId != null) {
			...
			if (!metodoPagamento.toString().endsWith(card.getCardType().toString())) {
				throw new ValidationException("O cartão cadastrado de id número " + card.getId() + ", foi cadastrado como um cartão de " + card.getCardType().toString() + ", não condiz com o método de pagamento fornecido, que é: " + metodoPagamento.toString());
			}
		}
	}
}
